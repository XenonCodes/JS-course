...
let producsDivEl = document.querySelector('.products');
let collectionButtonEl = document.querySelectorAll('button');
collectionButtonEl.forEach(button => {
...
function showCategory(category) {
    switch (category) {
        case 'phones':
            for (let phones of products.phones) {
                producsDivEl.innerHTML += getProductMarkup(phones);
            }
            break;
        case 'tablets':
            for (let tablets of products.tablets) {
                producsDivEl.innerHTML += getProductMarkup(tablets);
            }
            break;
        case 'tv':
            for (let tv of products.tv) {
                producsDivEl.innerHTML += getProductMarkup(tv);
            }
            break;
    }
}
